...
def extract_features(images, layer = 'fc7'):
    net = caffe.Net(caffe_root + 'models/VGG_CNN_S/VGG_CNN_S_deploy.prototxt',
                caffe_root + 'models/VGG_CNN_S/VGG_CNN_S.caffemodel',
                caffe.TEST)

    # input preprocessing: 'data' is the name of the input blob == net.inputs[0]
    transformer = caffe.io.Transformer({'data': net.blobs['data'].data.shape})
    transformer.set_transpose('data', (2,0,1))  #same sequence as caffenet(RGB -> BGR)
    #mean pixel is changed ilsvrc... to VGG_CNN_S_mean.npy which is generated from VGG_CNN_S.binaryphoto
    transformer.set_mean('data', np.load(caffe_root + 'python/caffe/imagenet/VGG_CNN_S_mean.npy').mean(1).mean(1)) # mean pixel
...
import h5py
#f.close()
f = h5py.File(h5_root+'test_image_fc7features_VGG.h5','w')
filenames = f.create_dataset('photo_id',(0,), maxshape=(None,),dtype='|S54')
feature = f.create_dataset('feature',(0,4096), maxshape = (None,4096)) # 4096
f.close()
...
batch_size = 4 # 16->4 for training time efficiency
...
for i in range(0, num_test, batch_size):
    ...
    f= h5py.File(h5_root+'test_image_fc7features_VGG.h5','r+')
